...
export default function ProjectMediaManager({ projectId, media, onMediaAdded, isLoading }: Props) {
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
          ...
          await api.projects.media.add(projectId.toString(), {
            media_url: url,
            media_type: type,
            is_featured: false,
  ...
  const handleDelete = async (mediaId: number) => {
    ...
    try {
      setIsSubmitting(true);
      await api.projects.media.delete(projectId.toString(), mediaId.toString());
      onMediaAdded();
